...
const NavBar = () =>{
    const {user} = useSelector(state => state.auth);
    return(
        <div className="fixed top-0 w-full h-16 bg-second flex items-center justify-start px-6 fontSignika shadow z-50">
            ...
            { (user === "no user" || !user) ? "" :
                <div className="w-1/3 text-lg text-white flex items-center justify-center space-x-12">
                    <div className="navItem cursor-pointer text-sm transition duration-200 hover:text-first flex flex-col items-center justify-center">
